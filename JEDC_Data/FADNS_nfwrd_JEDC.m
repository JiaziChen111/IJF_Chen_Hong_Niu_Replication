%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%% Forecasting the yield curve with FADNS_nfwrd (with only past macro-fin principal components)
...
for ij = 1:size(yield,1)
    EstMdlOLS = fitlm(Loading,yield(ij,:),'Intercept',false);
    NS(ij,:) = EstMdlOLS.Coefficients.Estimate';
end

%% Past variables loading
...
for i=1:78
   ...
   %%  extract the first two principal components of the forward-looking variables with pca 
    In_sample_whole = [non_fed_whole,FED_whole]; % 1992Q1-2002Q4
    
    %
    In_sample_whole_std=PC_std(In_sample_whole,39);
...
%%  Forecasting results
Rec_FADNS_nfwrd=For_result_save(y_h1,y_h2,y_h4,y_h8,y_h12,y_h16,y_h20);
save('.\Results\Rec_FADNS_nfwrd','Rec_FADNS_nfwrd');
