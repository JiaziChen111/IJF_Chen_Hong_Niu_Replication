...
if strcmp(boot,'BLOCK')
    [bsdata]=block_bootstrap((1:t)',B,w); % t by B matrix
else
    [bsdata]=stationary_bootstrap((1:t)',B,w); % t by B matrix
end

% All of these values cab be computed once
M0=size(losses,2); % number of models
...
for b=1:B
    meanworkdata=mean(losses(bsdata(:,b),:)); % l_i_bar from bootstrapped data: b
    for j=1:M0
...
% 2(b) depends on the det of models under considerations, so it will have to
% go in the loop
...
pvalsR=ones(M0,1);
for i=1:M0-1
    included=setdiff(1:M0,excludedR); %C = setdiff(A,B) for vectors A and B, returns the values in A that 
   % are not in B with no repetitions. C will be sorted.
    ...
    TR=max(max(abs(zdata)));%% 这里的zdata前面是否少了abs?
    pvalsR(i)=mean(empdistTR>TR); %所有的bootstrapped TR中大于 实际TR 的个数除以 B，即为p值
