...
%   Level factor is driven by the functional demographic factor + CPI
...
for i=1:245
   ...
   b = b.b';                                % b is length(Mt) by 3 matrix,i.e., 6 by 3
    ...
    phi_es_t=alpha*data_cpi_stand(621+i,i+1)+(1-alpha)*esi_history_orig(end,1);
    esi_history=[esi_history_orig; phi_es_t];
    esi_history=esi_history(370+i:end,1);
    else
     esi_history_orig=exp_smooth(data_cpi_stand(2:621+i,i+1),alpha); % month on month growth
     phi_es_t=alpha*data_cpi_stand(621+i,i+1)+(1-alpha)*esi_history_orig(end,1);
     esi_history=[esi_history_orig; phi_es_t];
     esi_history=esi_history(382+i:end,1);
     end
     
%     SP_L=[]; SP_L_trend=[];
%          SP_L=fitlm([z_86_1952(318+i:557+i,3),esi_history],Level); % Almost MHCV choose Kappa=2
%          SP_L_trend=SP_L.Fitted;
    ...
    SP_L=[]; SP_L_trend=[];
     ...
     elseif MHCV_L_DL(i,8)==2; % Kappa=3
         SP_L=fitlm([z_86_1952(318+i:557+i,3:4),esi_history],Level);
         SP_L_trend=SP_L.Fitted;
         else MHCV_L_DL(i,8)==3; % Kappa=5
         SP_L=fitlm([z_86_1952(318+i:557+i,3:6),esi_history],Level);
         SP_L_trend=SP_L.Fitted;   
     end         
...
%% Save the results
Rol_DNS_FD_L_u_RZIG=For_result_save(y_h3,y_h6,y_h12,y_h24,y_h36,y_h48,y_h60);
save('.\Results\Rol_DNS_FD_L_u_RZIG','Rol_DNS_FD_L_u_RZIG');