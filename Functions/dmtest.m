function [DM,p_value,varD] = dmtest(e1, e2, h)
...
% 
% -------------------------------------------------------------------------
% References % K. Bouman. Quantitative methods in international finance and  
...
if h > 1   
    gamma = zeros(h-1,1);   
    for i = 1:h-1       
    gamma(i) = (d(1+i:T)-ones(T-i,1).*dMean)' * (d(1:T-i)-ones(T-i,1).*dMean)/ T; % bugfix    
    end
...
%DM = real(dMean / sqrt ( (varD/T)));    
DM = dMean / sqrt ( (varD/T));    
%P_VALUE is calculated         
p_value = real(2*normcdf(-abs(DM)));
end
