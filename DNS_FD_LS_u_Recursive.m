%%  DNS_FD_LS_u: lamda is estimated by MLE method
%   Level factor and Slope factor for semiparametric regression, Recursive scheme
...
load 'Rec_CV_Slope.mat'
MHCV_S_DL=zeros(245,8);
for i=1:245
    MHCV_S_DL(i,:)=Rec_CV_Slope(2*i,:);
end

% CV_L_DL=xlsread('CV_selection_recursive_monthly','Level');
% MHCV_L_DL=zeros(245,8);
% for i=1:245
%     MHCV_L_DL(i,:)=CV_L_DL(2*i,4:11);
% end
% CV_S_DL=xlsread('CV_selection_recursive_monthly','Slope');
% MHCV_S_DL=zeros(245,8);
% for i=1:245
   ...
   b = b.b';                                % b is length(Mt) by 3 matrix,i.e., 6 by 3
    ...
    SP_L=[]; SP_L_trend=[];
     if MHCV_L_DL(i,8)==1; % Kappa=2
         SP_L=fitlm(z_86_1952(1:557+i,3),Level);
         SP_L_trend=SP_L.Fitted;
     elseif MHCV_L_DL(i,8)==2; % Kappa=3
         SP_L=fitlm(z_86_1952(1:557+i,3:4),Level);
         SP_L_trend=SP_L.Fitted;
         else MHCV_L_DL(i,8)==3; % Kappa=5
         SP_L=fitlm(z_86_1952(1:557+i,3:6),Level);
         SP_L_trend=SP_L.Fitted;   
     end
    ...
    SP_S=[]; SP_S_trend=[];
     if MHCV_S_DL(i,8)==1; % Kappa=2
         SP_S=fitlm(z_86_1952(1:557+i,3),Slope);
         SP_S_trend=SP_S.Fitted;
     elseif MHCV_S_DL(i,8)==2; % Kappa=3
         SP_S=fitlm(z_86_1952(1:557+i,3:4),Slope);
         SP_S_trend=SP_S.Fitted;
         else MHCV_S_DL(i,8)==3; % Kappa=5
         SP_S=fitlm(z_86_1952(1:557+i,3:6),Slope);
         SP_S_trend=SP_S.Fitted;   
     end    
...
%% Save the results
Rec_DNS_FD_LS_u=For_result_save(y_h3,y_h6,y_h12,y_h24,y_h36,y_h48,y_h60);
save('.\Results\Rec_DNS_FD_LS_u','Rec_DNS_FD_LS_u');